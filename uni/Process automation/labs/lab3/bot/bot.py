...
class FakeUpdate:
    def __init__(self, msg: telegram.Message | None, callback: CallbackQuery | None = None):
        self.message = msg
        self.callback_query = callback


def nn[T](in_: T | None) -> T:
    return cast(T, in_)
...
async def show_main_menu(
    update: Update | FakeUpdate,
    context: ContextTypes.DEFAULT_TYPE,
    message_text: str = "🛍️ *E-Shop Product Manager*\n\nWhat would you like to do?",
) -> None:
    keyboard = [
    ...
    if nn(update.callback_query):
        await nn(update.callback_query).edit_message_text(
            message_text, reply_markup=reply_markup, parse_mode="Markdown"
        )
    else:
        await nn(nn(update.message)).reply_text(message_text, reply_markup=reply_markup, parse_mode="Markdown")


async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Handle inline keyboard button presses"""
    query = nn(nn(update.callback_query))
    await query.answer()

    if nn(query.data) == "add":
        return await start_add_product(update, context)
    elif nn(query.data) == "edit":
        return await start_edit_product(update, context)
    elif nn(query.data) == "list":
        return await list_products(update, context)
    elif nn(query.data) == "view":
        return await start_view_product(update, context)
    elif nn(query.data) == "delete":
        return await start_delete_product(update, context)
    elif nn(query.data) == "cancel":
        await show_main_menu(update, context)
        return MAIN_MENU
...
async def start_add_product(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    query = nn(update.callback_query)
    await query.edit_message_text("➕ *Add New Product*\n\nPlease enter the product name:", parse_mode="Markdown")
    nn(context.user_data).clear()
    return ADD_NAME


async def add_name(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    nn(context.user_data)["name"] = nn(update.message).text
    await nn(update.message).reply_text("Enter the product description:")
    return ADD_DESCRIPTION


async def add_description(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    nn(context.user_data)["description"] = nn(update.message).text
    await nn(update.message).reply_text("Now enter the price (e.g., 19.99):")
    return ADD_PRICE


async def add_price(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    try:
        price = float(nn(update.message).text or "")
        if price < 0:
            await nn(update.message).reply_text("❌ Price cannot be negative. Please enter a valid price:")
            return ADD_PRICE
        nn(context.user_data)["price"] = price
        await nn(update.message).reply_text("Enter the stock quantity:")
        return ADD_STOCK
    except ValueError:
        await nn(update.message).reply_text("❌ Invalid price format. Please enter a number (e.g., 19.99):")
        return ADD_PRICE


async def add_stock(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    try:
        stock = int(nn(update.message).text or "")
        if stock < 0:
            await nn(update.message).reply_text("❌ Stock cannot be negative. Please enter a valid quantity:")
            return ADD_STOCK
        nn(context.user_data)["stock"] = stock

        keyboard = [[InlineKeyboardButton("Skip Image URL", callback_data="skip_image")]]
        reply_markup = InlineKeyboardMarkup(keyboard)

        await nn(update.message).reply_text("Enter an image URL (or click Skip):", reply_markup=reply_markup)
        return ADD_IMAGE
    except ValueError:
        await nn(update.message).reply_text("❌ Invalid stock format. Please enter a whole number:")
        return ADD_STOCK


async def add_image(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    if nn(update.callback_query):
        await nn(update.callback_query).answer()
        image_url = ""
        message = cast(telegram.Message, nn(update.callback_query).message)
    else:
        image_url = nn(update.message).text or ""
        message = nn(update.message)

    product = api.create_product(
        name=nn(context.user_data)["name"],
        description=nn(context.user_data)["description"],
        price=nn(context.user_data)["price"],
        stock=nn(context.user_data)["stock"],
        image_url=image_url,
    )
    ...
    if product:
        ...
        await show_main_menu(FakeUpdate(message), context)
    else:
...
# ===== LIST PRODUCTS =====
async def list_products(update: Update, _: ContextTypes.DEFAULT_TYPE) -> int:
    query = nn(update.callback_query)
    products = api.get_products()

    if not products:
        await query.edit_message_text("📋 *Product List*\n\n_No products found._", parse_mode="Markdown")
    else:
    ...
    reply_markup = InlineKeyboardMarkup(keyboard)
    await cast(telegram.Message, query.message).reply_text("Choose an action:", reply_markup=reply_markup)
...
async def start_view_product(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    for product in products:
        keyboard.append(
            [InlineKeyboardButton(f"{product['name']} (ID: {product['id']})", callback_data=f"view_{product['id']}")]
        )
    keyboard.append([InlineKeyboardButton("« Cancel", callback_data="cancel")])
    reply_markup = InlineKeyboardMarkup(keyboard)

    await query.edit_message_text(
        "🔍 *View Product*\n\nSelect a product to view:", reply_markup=reply_markup, parse_mode="Markdown"
    )
    return VIEW_PRODUCT
...
async def view_product_details(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if nn(query.data) == "cancel":
        await show_main_menu(update, context)
        return MAIN_MENU

    product_id = int(nn(query.data).split("_")[1])
...
async def start_edit_product(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    for product in products:
        keyboard.append(
            [InlineKeyboardButton(f"{product['name']} (ID: {product['id']})", callback_data=f"edit_{product['id']}")]
        )
    keyboard.append([InlineKeyboardButton("« Cancel", callback_data="cancel")])
    reply_markup = InlineKeyboardMarkup(keyboard)
...
async def edit_select_field(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    product_id = int(nn(query.data).split("_")[1])
    nn(context.user_data)["edit_product_id"] = product_id
...
async def edit_ask_value(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    field = nn(query.data).split("_")[1]
    if len(nn(query.data).split("_")) > 2:
        field = "_".join(nn(query.data).split("_")[1:])

    nn(context.user_data)["edit_field"] = field

    await query.edit_message_text(f"Please enter the new value for *{field}*:", parse_mode="Markdown")
    return EDIT_VALUE


async def edit_save_value(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    product_id = nn(context.user_data)["edit_product_id"]
    field = nn(context.user_data)["edit_field"]
    value = nn(update.message).text
    ...
    try:
        if field == "price":
            ...
            if value < 0:
                await nn(update.message).reply_text("❌ Price cannot be negative. Please try again:")
                return EDIT_VALUE
            update_data["price"] = value
        elif field == "stock":
            value = int(value or "")
            if value < 0:
                await nn(update.message).reply_text("❌ Stock cannot be negative. Please try again:")
                return EDIT_VALUE
            update_data["stock"] = value
        else:
            update_data[field] = value
    except ValueError:
        await nn(update.message).reply_text("❌ Invalid value format. Please try again:")
        return EDIT_VALUE

    product = api.update_product(product_id, **update_data)

    if product:
        await nn(update.message).reply_sticker(SUCCESS_STICKER)
        await nn(update.message).reply_text(
            f"✅ *Product Updated Successfully!*\n\nUpdated field: *{field}*", parse_mode="Markdown"
        )
    else:
        await nn(update.message).reply_sticker(FAIL_STICKER)
        await nn(update.message).reply_text("❌ Failed to update product.")

    await show_main_menu(FakeUpdate(update.message, CallbackQuery(nn(update.message))), context)
    nn(context.user_data).clear()
    return MAIN_MENU
...
async def start_delete_product(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    for product in products:
        keyboard.append(
            [InlineKeyboardButton(f"{product['name']} (ID: {product['id']})", callback_data=f"delete_{product['id']}")]
        )
    keyboard.append([InlineKeyboardButton("« Cancel", callback_data="cancel")])
    reply_markup = InlineKeyboardMarkup(keyboard)

    await query.edit_message_text(
        "🗑️ *Delete Product*\n\nSelect a product to delete:", reply_markup=reply_markup, parse_mode="Markdown"
    )
...
async def delete_confirm(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if nn(query.data).startswith("delete_"):
        product_id = int(nn(query.data).split("_")[1])
        product = api.get_product(product_id)
    ...
    elif nn(query.data).startswith("confirm_delete_"):
        product_id = int(nn(query.data).split("_")[2])
...
async def cancel(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    await nn(update.message).reply_text("Operation cancelled.")
    await show_main_menu(update, context)
    nn(context.user_data).clear()
    return MAIN_MENU
